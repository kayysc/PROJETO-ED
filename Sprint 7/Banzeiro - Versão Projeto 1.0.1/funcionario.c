...
void cancelarPorCPF(char ***assentos, int linhas, int colunas) {
    ...
    while (atual != NULL) {
        ...
        if (strcmp(cpf_limpo, cpf_atual_limpo) == 0) {
            ...
            printf("        Valor: R$ %.2f\n", (float)atual->preco);
            printf("        Assentos: ");
            for (int i = 0; i < MAX_PASSAGENS && atual->assentos[i][0] != '\0'; i++) {
            ...
            if (confirmacao == 's' || confirmacao == 'S') {
                ...
                for (int k = 0; k < MAX_PASSAGENS && atual->assentos[k][0] != '\0'; k++) {
                ...
                // Registro da operação no histórico
                char descricaoOperacao[550];
                snprintf(descricaoOperacao, sizeof(descricaoOperacao), "Cancelamento de compra por CPF: | ID - %d | Nome - %s | CPF - %s", atual->notaFiscal, atual->nome, atual->cpf);
                registrarOperacao(userLogado, descricaoOperacao);
    ...
    while (atual != NULL) {
        if (atual->notaFiscal == id) {
            ...
            if (confirmacao == 's' || confirmacao == 'S') {
                ...
                // Registro da operação no histórico
                char descricaoOperacao[550];
                snprintf(descricaoOperacao, sizeof(descricaoOperacao), "Cancelamento de compra por ID: | ID - %d | Nome - %s | CPF - %s", atual->notaFiscal, atual->nome, atual->cpf);
                registrarOperacao(userLogado, descricaoOperacao);

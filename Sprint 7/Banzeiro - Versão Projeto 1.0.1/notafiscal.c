...
void finalizarCompra(int totalAssentos, Rota rota, char assentosEscolhidos[MAX_PASSAGENS][5], const char *nome, const char *cpf, char ***assentos) {
    ...
    while (1) { // Loop while para controle de fluxo
        ...
        if (opcao == 'S' || opcao == 's') {
            ...
            char descricaoOperacao[600];
            snprintf(descricaoOperacao, sizeof(descricaoOperacao), "Venda de Passagem | Nome: %s | CPF: %s | Rota: %s -> %s | Data: %s | Horario: %s | Assentos: ", nome, cpf, rota.origem, rota.destino, rota.data, rota.horario);

            for (int i = 0; i < totalAssentos; i++) {
                strcat(descricaoOperacao, assentosEscolhidos[i]);
                strcat(descricaoOperacao, " ");
            }
